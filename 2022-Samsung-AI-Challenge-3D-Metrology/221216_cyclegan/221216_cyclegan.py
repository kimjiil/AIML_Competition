...
class cycleGAN_model(nn.Module):
    ...
    def model_valid(self, real_A, real_B):
        ...
        with torch.no_grad():
            ...
            true = (real_B * 255).type(torch.int8).float()
            rmse_loss = torch.sqrt(self.criterion['mse'](fake_B, true))
...
for fold_i, data_loaders in enumerate(folds_dataloader_simtotrain):
