...
def valid(model, valid_dataloader, device):
    rmse_list = []
    for step_i, data_tuple in enumerate(tqdm(iter(valid_dataloader))):
...
def training(model, train_dataloader, valid_dataloader, epochs, device, fold_i, type, checkpoint_path=None):
    ...
    for epoch in range(epochs):
        loss_list = [[], [], []]
        for step_i, data_tuple in enumerate(tqdm(iter(train_dataloader))):
            break
            real_a = data_tuple[0].to(device, non_blocking=True)
            real_b = data_tuple[1].to(device, non_blocking=True)
...
model_dic = {
    'semtodepth': cycleGAN_model(1, gan_mode='wgan_gp'),
    'simtotrain': cycleGAN_model(1, gan_mode='wgan_gp')
}
...
class test_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(data, str):
            ...
            img = PIL.Image.open(data).convert("L")
...
print()
train_data_path = []
for i in chunk_dic['train']:
    train_data_path.extend(i)

_test_dataset = test_dataset(train_data_path)
_test_dataloader = DataLoader(dataset=_test_dataset,batch_size=3,num_workers=0,shuffle=True)

def test_validation(model_dic, dataloader, device):
    import matplotlib.pyplot as plt

    std_model = model_dic['semtodepth'] # simulation sem to simulation depth
    stt_model = model_dic['simtotrain'] # simulation sem to train sem

    std_model.model_load("./typesemtodepth_f0_best_model.pth", device=device)
    stt_model.model_load("./typesimtotrain_f0_best_model.pth", device=device)
    std_model.to(device)
    stt_model.to(device)

    img_mean_list = [[], [], [], []]
    for i, data_tuple in enumerate(tqdm(dataloader)):
        train_sem_img = data_tuple[0].to(device, non_blocking=True)
        avg_depth = data_tuple[1].to(device, non_blocking=True)

        simulation_sem_img = stt_model.Gen['B'](train_sem_img) #train sem to simulation sem
        depth_img = std_model.Gen['A'](simulation_sem_img)

        np_depth_img = (depth_img * 255).type(torch.uint8).detach().cpu().numpy()
        np_avg_depth = list(map(round, avg_depth.detach().cpu().numpy(), [-1]*avg_depth.shape[0]))
        for img_i, (_img, _d) in enumerate(zip(np_depth_img, np_avg_depth)):
            img_mean_list[int(_d % 100 // 10 - 1)].append(np.mean(_img))

    plt.figure(figsize=(5, 3))
    for i, l in enumerate(img_mean_list):
        plt.hist(l, bins=100, label=str((i+1) * 10 + 100), alpha=0.5, color=plt.cm.tab20c(4*i), density=True)
...
test_validation(model_dic, _test_dataloader, device=cfg['device'])
